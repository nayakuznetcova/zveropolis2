...
public class DogMenu implements State {
    ...
    public DogMenu(@Lazy TelegramBotSendMessage telegramBotSendMessage, ShelterRepository shelterRepository, SubscriberRepository subscriberRepository, @Lazy Relocation relocation) {
    ...
    public void sendMessageAtCallback(Update update) {
        ...
        switch (data) {
            ...
            case HOW_TAKE_PET -> {
                subscriberRepository.putStateBot(chatId, StateBot.INFO_DOG_SHELTER_MENU);
                State state = relocation.getState(chatId);
                state.execute(update);
            }
            case SEND_REPORT -> {
                subscriberRepository.putStateBot(chatId, StateBot.REPORT_DOG_MENU);
                State state = relocation.getState(chatId);
                state.execute(update);
            }
            case CALL_VOLUNTEER -> {
